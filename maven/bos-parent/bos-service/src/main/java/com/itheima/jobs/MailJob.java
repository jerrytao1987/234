	...
	public void execute() {
		...
		try {
			...
			if(null != list && list.size() > 0){
				...
				//mailProps.setProperty("mail.transport.protocol", "SMTP");//发邮件的协议
				//mailProps.setProperty("mail.host", "localhost");//发送邮件的服务器地址
				...
				for(Workbill workbill : list){
					...
					// 设置收件人
					InternetAddress to = new InternetAddress("lucy@dengting.com");
