...
<script type="text/javascript">
	function delProFromCart(pid) {
		if (confirm("您是否确定要删除?")) {
			location.href = "${pageContext.request.contextPath }/product?method=delProFromCart&pid="
					+ pid;
		}
	}
	
	function clearCart(){
		if (confirm("您是否确定要清空购物车?")) {
...
<body>
	...
	<c:if test="${empty cart.cartItems }">
		<div style="background:url(${pageContext.request.contextPath}/images/cart-empty.png) no-repeat center;height:300px">
			
		</div>
		<div style="text-align:center;"><a href="${pageContext.request.contextPath }/">返回首页</a></div>
	</c:if>
	<c:if test="${!empty cart.cartItems }">
		<div class="container">
			<div class="row">
						...
						<tbody>
							...
							<c:forEach items="${cart.cartItems }" var="entry">
								...
								<tr class="active">
									...
									<td width="10%"><input type="text" name="buyNum"
										maxlength="4" size="10" value="${entry.value.buyNum}">
									</td>
									<td width="15%"><span class="subtotal">￥${entry.value.subtotal}</span>
									</td>
									<td><a href="javascript:;" class="delete"
										onclick="delProFromCart('${entry.key }')">删除</a></td>
								</tr>
			...
			<div style="margin-right: 130px;">
				...
				<div
					...
					<a
						href="${pageContext.request.contextPath }/product?method=submitOrder"> <input type="button" width="100"
