...
<SCRIPT language=javascript>
	...
	function selectUser(user_id,user_name){
		...
		doc.getElementById("user_id").value=user_id;
											...
											<TBODY>
												<TR>
													...
													<TD><INPUT class=textbox id=sChannel2
														style="WIDTH: 80px" maxLength=50 name="user_name"
												...
												<TR
													...
													<TD>
														<s:if test="#parameters.select==null">
															<a href="${pageContext.request.contextPath }/UserAction_toEdit?user_id=<s:property value='#user.user_id'/>">修改</a>
																&nbsp;&nbsp; 
															<a href="${pageContext.request.contextPath }/customerServlet?method=delete&custId=${customer.cust_id}">删除</a>
														</s:if>
														<s:else>
															<input type="button" value="选择" onclick="selectUser('<s:property value="#user.user_id" />','<s:property value="#user.user_name" />')"/>
														</s:else>
													</TD>
												</TR>
								...
								<TD><SPAN id=pagelink>
										<DIV
											...
											共[<B><s:property value="#pageBean.totalCount" /></B>]条记录,[<B><s:property
													value="#pageBean.totalPage" /></B>]页 ,每页显示 <select onchange="changePageSize(this.value)"
												name="pageSize">
